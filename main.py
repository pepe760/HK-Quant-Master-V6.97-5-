...
month_options_html = ""
for m in sorted(list(entry_months), reverse=True):
    month_options_html += f'<option value="{m}">{m}</option>'
...
if not recent_signals:
    html_content += f"""
                    <div class="card p-6 text-center text-slate-500">
                        ...
                        <p class="text-xs mt-2 text-slate-400">寧願空手，絕不接刀。</p>
                    </div>
"""
else:
                    ...
                    <div class="card p-4 cursor-pointer hover:bg-slate-700 transition shadow" onclick="loadChart('{sig['Trade_ID']}', 'myChartScan')">
                        ...
                        <div class="bg-slate-800 p-2 rounded mb-3">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2">
                                <div class="info-badge"><span class="text-slate-400 block mb-1">股息殖利率</span><span class="{div_color} text-sm">{sig['div_yield']}%</span></div>
                                <div class="info-badge"><span class="text-slate-400 block mb-1">近期業績動能</span><span class="{earn_color} font-bold text-sm">{sig['earn_label']}</span></div>
                            </div>
                            <div class="grid grid-cols-3 gap-2">
                                <div class="info-badge"><span class="text-slate-400 block mb-1">P/E</span><span class="text-slate-300 font-bold text-sm">{sig['pe_ratio']}</span></div>
                                <div class="info-badge"><span class="text-slate-400 block mb-1">P/B</span><span class="text-slate-300 font-bold text-sm">{sig['pb_ratio']}</span></div>
                                <div class="info-badge"><span class="text-slate-400 block mb-1">ROE</span><span class="text-slate-300 font-bold text-sm">{sig['roe']}%</span></div>
                            </div>
                        </div>

                        <div class="grid grid-cols-5 gap-1 text-center bg-slate-900 p-2 rounded border border-slate-700">
                            <div><div class="text-[10px] text-slate-500">進場價</div><div class="font-bold text-white">${sig['Entry_Price']}</div></div>
                            <div><div class="text-[10px] text-red-400">防守線</div><div class="font-bold text-red-400">${sig['SL']}</div></div>
                            <div><div class="text-[10px] text-green-400">止盈</div><div class="font-bold text-green-400">${sig['TP']}</div></div>
                            <div class="bg-slate-800 rounded"><div class="text-[10px] text-blue-300">最新價</div><div class="font-bold text-blue-300">${sig['current_price']}</div></div>
                            <div class="bg-slate-800 rounded"><div class="text-[10px] {pnl_clr}">浮動損益</div><div class="font-bold {pnl_clr}">{'+' if unrealized_pnl>0 else ''}{unrealized_pnl:.1f}%</div></div>
                        </div>
                    </div>
"""
...
html_content += f"""
                ...
                <div class="lg:w-3/5 flex flex-col">
                    <div class="card p-4 mb-4 flex justify-between items-center bg-gradient-to-r from-slate-800 to-slate-900">
                        <h3 class="text-xl font-bold text-white">圖表預覽</h3>
                    </div>
        ...
        <div id="tab-5y" class="tab-content hidden">
            ...
            <div id="filteredSummaryPanel" class="hidden mb-4 p-4 border border-blue-500 bg-blue-900/30 rounded-lg flex justify-between items-center">
                <div>
                    <span class="text-blue-300 font-bold mr-2">📅 獨立期間篩選統計：</span>
                    <span class="text-slate-300 text-sm">已平倉筆數 <span id="filteredCount" class="font-bold text-white">0</span> 筆</span>
                </div>
                <div class="text-right">
                    <span class="text-slate-400 text-xs block">該期間累積複利淨利</span>
                    <span id="filteredPnL" class="text-xl font-black text-green-400">0.00%</span>
                </div>
            </div>
            ...
            <div class="mt-4 pr-2">
                <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                    ...
                    <div class="flex gap-2">
                        <select id="filterMonth" onchange="filterTable()" class="bg-slate-900 text-yellow-400 font-bold p-2 border border-yellow-600 rounded text-sm focus:outline-none focus:border-blue-500">
                    ...
                    <table id="logTable" class="w-full text-left border-collapse whitespace-nowrap" data-sort-dir="desc">
                        <thead class="bg-slate-800">
                            <tr class="text-slate-300 text-sm border-b border-slate-700 shadow-md">
                                ...
                                <th class="p-3 text-center">觸發時財務快照</th>
            ...
            <div class="card p-6 border-t-4 border-yellow-400 shadow-xl">
                ...
                <div class="mt-6 bg-slate-800 p-5 rounded-lg text-sm text-slate-300 leading-relaxed border border-slate-600">
                    ...
                    <ul class="list-disc pl-5 mt-4 space-y-2 font-bold text-white">
                        ...
                        <li><span class="text-red-400">紅線 (RSI 逆勢)</span>：套用 V6.8 終極濾網後，在熊市恐慌末期安全榨取反彈利潤。</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function filterTable() {{
            ...
            if (isFiltering && closedCount > 0) {{
                ...
                let finalPnL = (visiblePnL - 1) * 100;
                let pnlEl = document.getElementById('filteredPnL');
                pnlEl.innerText = (finalPnL >= 0 ? '+' : '') + finalPnL.toFixed(2) + '%';
                pnlEl.className = finalPnL >= 0 ? 'text-xl font-black text-green-400' : 'text-xl font-black text-red-400';
                document.getElementById('filteredCount').innerText = closedCount;
            }} else {{
...
# 同時輸出 index.html 給 GitHub Pages 使用
