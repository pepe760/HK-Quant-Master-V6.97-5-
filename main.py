#@title HK Quant Master V10.8 (Ultimate: UI Polished & AI Prompt Enhanced)
    ...
    for i in tqdm(range(start_idx, end_idx), desc=scenario_name):
        ...
        for ticker in closes.columns:
            ...
            if signal:
                ...
                # [V10.8 更新] 格式化基本面輸出，與 User 要求完全一致
...
<head>
    ...
    <title>HK Quant Master V10.8 - 終極打擊面板</title>
    <style>
...
<body class="p-4">
    <div class="max-w-7xl mx-auto w-full">
        <div class="card p-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center shadow-lg border-b-4 border-blue-500">
            <div>
                <h1 class="text-3xl font-black text-white mb-1">HK Quant Master <span class="text-blue-500">V10.8 Pro</span></h1>
                <p class="text-slate-400 text-sm">無限制掃描 | 智能雙向排序 | 雙 AI 剪貼驗證 | 訪客看板強化</p>
        ...
        <div id="tab-scan" class="tab-content block">
            <div class="card p-6 mb-4 bg-gradient-to-r from-slate-800 to-slate-900 border-l-4 border-blue-500 shadow-xl">
                ...
                <p class="text-slate-400 text-sm">點擊 ✨ Gemini 或 🤖 Grok，系統將自動複製該股最新數據並開啟 AI 分析網頁。</p>
            </div>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead class="bg-slate-800">
                        <tr class="text-slate-300 text-sm border-b border-slate-700">
                            ...
                            <th class="p-3">基本面快照</th> <th class="p-3 text-right sortable" onclick="sortTableGen('activeTableBody', 4)">進場價 ↕</th>
                            ...
                            <th class="p-3 text-center">🤖 AI 雙重診斷</th>
                        </tr>
                    </thead>
        ...
        <div id="tab-manual" class="tab-content hidden card p-8 leading-relaxed text-slate-300">
            <h2 class="text-2xl font-bold text-blue-400 mb-6 border-b border-slate-700 pb-2">📖 V10.8 系統說明書 (User Manual)</h2>
        ...
        <div class="text-center mt-12 mb-4 flex flex-col items-center justify-center">
            ...
            <img src="https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fgithub.com%2Fhk-quant-master&count_bg=%232563EB&title_bg=%231E293B&icon=&icon_color=%23E7E7E7&title=Views&edge_flat=false" alt="Hits" style="transform: scale(1.5);">
        </div>
...
1. 【技術條件雙重驗證】：請幫我核實上述的「技術觸發條件」是否真實反映在該股最新的 K 線圖表與大盤環境中？
2. 【基本面與籌碼/消息面】：這家公司近期的基本面是否穩健？有沒有任何隱藏的地雷（如盈警、配股、大行降評、政策打壓）導致它被錯殺或假突破？
3. 【最終實戰建議】：綜合以上，這個量化訊號目前值得跟單買進嗎？請給出明確的「強烈買進 (BUY)」、「觀望 (HOLD)」或「避開 (AVOID)」結論，並說明停損建議。`;
        }}

        function copyAndOpenGemini(tradeId, isRelaxed) {{
            let row;
            if (isRelaxed) {{
        ...
        function buildActionBtns(row) {{
            const tvUrl = `https://www.tradingview.com/chart/?symbol=${{row.TV_Ticker}}`;
            const grokUrl = `https://grok.com/?q=${{encodeURIComponent(buildPromptText(row))}}`;
            return `<div class="flex gap-1 justify-center">
                <button onclick="copyAndOpenGemini('${{row.Trade_ID}}', false); event.stopPropagation();" class="gemini-btn text-[10px] font-bold px-2 py-1 rounded text-[#c084fc] hover:text-white transition-colors" title="複製提示詞並開啟 Gemini">✨ Gemini</button>
                <a href="${{grokUrl}}" target="_blank" class="grok-btn text-[10px] font-bold px-2 py-1 rounded text-red-400 hover:text-white transition-colors" onclick="event.stopPropagation()" title="Grok AI 專業分析">🤖 Grok</a>
            </div>`;
        }}
        ...
        function buildActionBtnsSimple(row) {{
            ...
            // Map simple row to match main logic
            let mappedRow = {{
                Ticker: row.ticker, Stock_Name: row.name, TV_Ticker: row.tv_ticker,
                Type: "RSI 寬鬆觀察", current_price: row.price, RSI_Value: row.rsi,
                Lot_Size: row.lot_size, Lot_Cost: row.lot_cost, financial_data: ""
            }};
            
            const grokUrl = `https://grok.com/?q=${{encodeURIComponent(buildPromptText(mappedRow))}}`;
            return `<div class="flex gap-1 justify-center">
                <button onclick="copyAndOpenGemini('${{row.ticker}}', true); event.stopPropagation();" class="gemini-btn text-[10px] font-bold px-2 py-1 rounded text-[#c084fc] hover:text-white transition-colors" title="複製提示詞並開啟 Gemini">✨ Gemini</button>
                <a href="${{grokUrl}}" target="_blank" class="grok-btn text-[10px] font-bold px-2 py-1 rounded text-red-400 hover:text-white transition-colors" onclick="event.stopPropagation()" title="Grok AI 專業分析">🤖 Grok</a>
            </div>`;
        }}
        ...
        function buildTvLink(tvTicker, displayTicker) {{
            return `<a href="https://www.tradingview.com/chart/?symbol=${{tvTicker}}" target="_blank" class="text-blue-400 hover:text-blue-300 underline underline-offset-2" onclick="event.stopPropagation()">${{displayTicker}}</a>`;
        }}
...
print(f"🎉 成功！生成 HK Quant Master V10.8 終極 Dashboard：index.html")
try:
